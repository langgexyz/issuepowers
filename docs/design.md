...
启动 requirements-understanding 对话：
    ...
    ↓
分流：
    ├─ 拆分 → split-proposal.md → 人 confirm → 创建子 issue → 各自跑 understanding
    └─ 不拆 → understanding.md confirmed
    ↓
返回（state=in-progress 或 split-into-children）
    ↓
...
4. 不可逆数据变更 → 强制 pg_dump 备份 + 人 gate
...
## 十、Skill / Command / Hook / Template 清单
...
3. **风险评级**：保留为 understanding.md 内的字段（人填），还是 Agent 自评？
4. **`/rollback` 触发权限**：deliverable-check 失败时 Agent 自动触发，还是必须人触发？
