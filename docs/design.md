...
2. **回滚就绪**：rollback playbook 在 merge 前已生成；用户验证失败可一键回滚。
3. **并行物理隔离 + 串行合并**：worktree 物理并行开发，`.merge-lock` 保证合并串行 → 回滚永远干净。
4. **围绕用户的语言**：面向用户的产物（understanding.md / user-validation-feedback.md / 验证通知 / 错误提示）必须用普通话，避免行业黑话（UAT / smoke test / deliverable / rollback playbook 等）。技术术语只在 skill 内部和 Agent 视角文档中保留。**用户最终都是非技术的提单人**，issuepowers 的入口（理解需求）和出口（验证反馈）都是用户在用，必须能让用户没有阅读门槛。
...
用户验证失败则不同 —— 那是「需求 vs 实现」对不上，可能小事可能大事，由人决定 fix-forward 还是 rollback（见 §4 状态机）。
